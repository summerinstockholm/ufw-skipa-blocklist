...
- если на сервере `nftables.service` **включён** (`enabled`) или **активен** (`active`), `install-skipa-banlist.sh` и `update-skipa-banlist.sh` **завершатся с ошибкой** и не будут вносить изменения;
- существующие правила UFW не удаляются и не переписываются;
- managed-блок добавляется в `/etc/ufw/before.rules`, поэтому проверять результат нужно через `ufw show raw` и просмотр самого файла, а не только через `ufw status`.

## Trust model

По умолчанию updater тянет список из upstream-репозитория по raw URL на ветку `main`.

Это означает:

- любое изменение upstream `main` будет использовано на следующем обновлении;
- если upstream-список вырастет или изменится, это отразится на firewall после очередного запуска updater-а;
- если upstream временно недоступен, обновление завершится ошибкой, а уже применённый ранее managed-блок останется как есть.

Практический режим использования:

- сначала установить и проверить решение на одном хосте вручную;
- посмотреть `ufw show raw` и содержимое managed-блока в `/etc/ufw/before.rules`;
